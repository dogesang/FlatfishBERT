#!/usr/bin/env python3
"""
FlatfishBert evaluation script — save per-sample predictions for the
MMseqs2 leakage-sensitivity re-evaluation (Section 3.3, Table S6).

Environment: flatfishbert
"""
...
EXP4_MODEL_PATH = Path(os.getenv(
    "FLATFISH_EXP4_MODEL",
    str(PROJECT_ROOT / "checkpoints" / "flatfishbert" / "Exp4_cross_dedup" / "final_model")
))

TOKENIZER_PATH = Path(os.getenv(
    "FLATFISH_TOKENIZER_PATH",
    str(PROJECT_ROOT / "pretrain" / "tokenizer" / "flatfish_tokenizer")
))
...
def evaluate_flatfishbert(model_path, tokenizer_path, test_data):
...
def main():
    ...
    results = evaluate_flatfishbert(str(EXP4_MODEL_PATH), str(TOKENIZER_PATH), test_data)

    output_dir = PROJECT_ROOT / "similarity_audit" / "results" / "filtered_evaluation"
    output_dir.mkdir(parents=True, exist_ok=True)

    output_path = output_dir / "flatfishbert_predictions.json"
