#!/usr/bin/env python3
"""
Task N 数据准备脚本 - 反向互补增强版本

任务描述:
- 基于 Task N baseline，对少数类进行反向互补数据增强
- 增强类别: tRNA, rRNA, snRNA, snoRNA (不增强lncRNA_exon)
...
# 需要增强的少数类
AUGMENT_LABELS = ['tRNA', 'rRNA', 'snRNA', 'snoRNA']
...
def load_task_n_baseline(data_dir: Path) -> Tuple[List[Dict], List[Dict], List[Dict]]:
    """加载Task N baseline数据
    ...
    """
    logger.info(f"加载Task N baseline数据: {data_dir}")
    ...
    if not train_path.exists():
        raise FileNotFoundError(f"Task N训练数据不存在: {train_path}")
...
def print_augmentation_summary(
    ...
    # 训练集对比
    logger.info("\n【训练集】")
    logger.info(f"{'类别':<15} {'原始':>10} {'增强后':>10} {'增加':>10}")
    logger.info("-" * 50)
    ...
    for label in TASK_N_LABELS:
        ...
        diff = aug - orig
        logger.info(f"{label:<15} {orig:>10,} {aug:>10,} {'+' + str(diff) if diff > 0 else diff:>10}")

    logger.info("-" * 50)
    logger.info(f"{'总计':<15} {len(original_train):>10,} {len(augmented_train):>10,} {'+' + str(len(augmented_train) - len(original_train)):>10}")
    ...
    logger.info("-" * 50)
    logger.info(f"{'总计':<15} {len(original_eval):>10,} {len(augmented_eval):>10,} {'+' + str(len(augmented_eval) - len(original_eval)):>10}")
...
def prepare_task_n_augmented():
    """准备Task N增强数据"""

    logger.info("=" * 70)
    logger.info("Task N 反向互补数据增强")
    logger.info("=" * 70)
    logger.info("\n增强策略:")
    logger.info(f"  - 增强类别: {AUGMENT_LABELS}")
    logger.info(f"  - 不增强类别: lncRNA_exon (多数类)")
    logger.info(f"  - 训练集: 增强")
    logger.info(f"  - 验证集: 增强")
    logger.info(f"  - 测试集: 不增强 (保持原样用于公平评估)")

    # 加载baseline数据
    baseline_dir = DATA_OUTPUT_ROOT / "Task_N"
    train_data, eval_data, test_data = load_task_n_baseline(baseline_dir)
    ...
    # 保存数据
    logger.info("\n保存增强数据...")
    output_dir = DATA_OUTPUT_ROOT / "Task_N_aug"
    ...
    metadata = {
        ...
        'augmentation': {
            ...
            'non_augmented_labels': ['lncRNA_exon'],
            'train_augmented': True,
            'eval_augmented': True,
            'test_augmented': False,
            'reason': '对少数类(tRNA/rRNA/snRNA/snoRNA)进行反向互补增强，提升类别平衡性'
        },
        'imbalance_handling': {
    ...
    logger.info("\n" + "=" * 70)
    logger.info("✅ Task N 反向互补增强数据准备完成!")
...
def main():
    parser = argparse.ArgumentParser(
        description='Task N 数据准备 - 反向互补增强版本'
    )
    args = parser.parse_args()

    prepare_task_n_augmented()
